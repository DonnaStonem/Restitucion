...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    // Obtener el ID del abogado desde el formulario
    $id_abogado = $_POST['id_abogado'];

    // Consulta SQL para obtener la célula de trabajo donde pertenece el abogado
    $query = "SELECT * FROM celulas_trabajo WHERE id_abogado = $1";
    $result = pg_query_params($conn, $query, array($id_abogado));
    ...
    if (pg_num_rows($result) > 0) {
        ...
        echo "<p>No se encontraron resultados para el ID del Abogado: $id_abogado</p>";
    }
}
